...
export default async function(app: FastifyInstance) {
  app.get('/menu-from-session/:id', async (req, res) => {
    ...
    const menu = await prisma.menu.findUnique({ where: { id: session.menu_id } })
    if (!menu)
      throw new NotFoundError('Menu not found.')

    const categories = await prisma.category.findMany({ where: { menu_id: session.menu_id }, orderBy: { pos: 'asc' } })
